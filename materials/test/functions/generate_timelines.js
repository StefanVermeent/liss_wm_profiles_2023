...
generate_timeline_variables_rspan = function(stimSet, normalSet, mirrorSet, setSize, task){
  var stims = shuffleWithoutReplacement(arr = stimSet, size = setSize);
  var rot_correct = shuffleWithReplacement(arr = [true, false], size = setSize, immediate_reps = true)

  var stimRot = []
  for (var i = 0; i < setSize; i++) {
    if(rot_correct[i] == true) {
      stimRot.push(shuffleWithoutReplacement(normalSet, 1))
    } else {
      stimRot.push(shuffleWithoutReplacement(mirrorSet, 1))
    }
  }
  ...
  return({stim_arr: stims, stim_rot: stimRot, rot_correct: rot_correct, key_answer: answer_i, setSize: setSize, task: task})
}
