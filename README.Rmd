...
milestones <- gert::git_tag_list() |> 
  ...
  mutate(
    ...
    code = ifelse(name == "Data Access", str_remove_all(code, "^code\\s") |> str_replace_all(string = _, "\\|\\>", "\\|\\>\\\n"), "Not applicable"),
    link_to_git = ifelse(name == 'Data Access', 
                         paste0(get_git_url(), "/blob/", branch, "/", commit, ".R"),
                         paste0(get_git_url(), "/tree/", commit)
    ),
    hash = ifelse(name != "Data Access", "Not applicable", hash),
...
milestones |> 
  pwalk(function(name,commit,author,time,description,hash,code,branch,link_to_git){
